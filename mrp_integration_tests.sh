...
expect_status 200
if have_jq; then
  # Versuche Token mit jq zu lesen (Feldnamen: token, accessToken oder data.token)
  T=$(printf '%s' "$LAST_BODY" | "$JQ_BIN" -r '(.token // .accessToken // .data.token // empty)' 2>/dev/null || true)
else
  # Fallback: Versuche Token mit sed zu lesen (für User ohne jq)
  T=$(echo "$LAST_BODY" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
fi

if [[ -n "$T" && "$T" != "null" ]]; then
  export TOKEN="$T"
  log "Captured TOKEN successfully"
else
  ko "CRITICAL: Could not extract TOKEN from login response."
  echo "Body was: $LAST_BODY"
  exit 1
fi
...
# Capture RATING_ID if returned
if have_jq; then
  rid="$(printf '%s' "$LAST_BODY" | "$JQ_BIN" -r '(.id // .ratingId // .data.id // empty)' 2>/dev/null || true)"
  if [[ -n "${rid:-}" && "$rid" != "null" ]]; then RATING_ID="$rid"; export RATING_ID; log "Captured RATING_ID=$RATING_ID"; fi
fi
